...
if (!function_exists('ci_is_https')) {
	function ci_is_https()
	{
...
if (!function_exists('ci_detect_base_url')) {
	function ci_detect_base_url()
	{
		...
		return $scheme . '://' . $host . $port . ($scriptDir !== '' ? $scriptDir . '/' : '/');
	}
}

$config['base_url'] = ci_detect_base_url();

$publicBaseUrl = trim((string)(getenv('SRMS_PUBLIC_BASE_URL') ?: $config['base_url']));
$publicBaseUrl = $publicBaseUrl !== '' ? rtrim($publicBaseUrl, '/') : rtrim($config['base_url'], '/');
$config['public_app_base_url'] = $publicBaseUrl;

$privacyPolicyUrl = trim((string)getenv('SRMS_PRIVACY_POLICY_URL'));
if ($privacyPolicyUrl === '') {
	$privacyPolicyUrl = $publicBaseUrl . '/dataprivacy';
}
$config['public_privacy_policy_url'] = rtrim($privacyPolicyUrl, '/');

$config['enable_hooks'] = TRUE;
$config['maintenance_mode'] = FALSE;
$config['mobile_release_public_path'] = 'Download';
$config['mobile_release_manifest'] = FCPATH . trim($config['mobile_release_public_path'], '/\\') . DIRECTORY_SEPARATOR . 'release.json';
...
$config['csrf_token_name'] = 'nexam_csrf_token';
$config['csrf_cookie_name'] = 'nexam_csrf_cookie';
