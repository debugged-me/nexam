...
//                echo 'Adjusted Formula is ', $value, PHP_EOL;
            ...
            switch ($rel["Type"]) {
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/theme":
                    ...
                    if (is_object($xmlTheme)) {
                        ...
                        if (isset($xmlTheme->themeElements) && isset($xmlTheme->themeElements->clrScheme)) {
        ...
        foreach ($rels->Relationship as $rel) {
            switch ($rel["Type"]) {
                ...
                case "http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument":
                    ...
                    $sharedStringsTarget = ($xpath !== null && isset($xpath['Target'])) ? "$dir/$xpath[Target]" : "$dir/sharedStrings.xml";
                    $xmlStrings = false;
                    if ($sharedStringsTarget !== '') {
                        $xmlStringsData = $this->getFromZipArchive($zip, $sharedStringsTarget);
                        if ($xmlStringsData !== false) {
                            $xmlStrings = simplexml_load_string($this->securityScan($xmlStringsData), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());  //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
                        }
                    }
                    ...
                    $stylesTarget = ($xpath !== null && isset($xpath['Target'])) ? "$dir/$xpath[Target]" : "$dir/styles.xml";
                    $xmlStyles = false;
                    if ($stylesTarget !== '') {
                        $xmlStylesData = $this->getFromZipArchive($zip, $stylesTarget);
                        if ($xmlStylesData !== false) {
                            $xmlStyles = simplexml_load_string($this->securityScan($xmlStylesData), 'SimpleXMLElement', PHPExcel_Settings::getLibXmlLoaderOptions());
                        }
                    }
                    //~ http://schemas.openxmlformats.org/spreadsheetml/2006/main");
    ...
    //                                            echo 'String', PHP_EOL;
                                                if ((string)$c->v != '') {
                                                    $value = self::getArrayItem($sharedStrings, intval($c->v));
                                                    if ($value === null) {
                                                        $value = (string) $c->v;
                                                    }
