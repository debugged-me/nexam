...
.admin-tag{font-size:.7rem;font-weight:500;letter-spacing:.2em;text-transform:uppercase;color:var(--gold);background:rgba(184,134,11,.08);border:1px solid rgba(184,134,11,.15);padding:.25rem .7rem;border-radius:6px;display:inline-block;margin-bottom:1.5rem}
@media(max-width:768px){body.login-page{padding:0;align-items:flex-start}.login-wrap{grid-template-columns:1fr;max-width:100%;border-radius:0;min-height:100vh;box-shadow:none}.login-panel{padding:1.2rem 1.5rem;min-height:auto;flex-direction:row;align-items:center;gap:.8rem;justify-content:flex-start}.panel-crown{font-size:1.8rem;margin-bottom:0}.panel-badge{display:none}.panel-title{font-size:1.15rem;margin-bottom:0}.panel-title em{display:inline;margin-top:0;margin-left:.35rem}.panel-tagline{display:none}.panel-footer{display:none}.login-form-wrap{padding:2rem 1.5rem 2.5rem}.form-group{margin-bottom:1.1rem}.form-input{padding:1rem 1.1rem;border-radius:10px;font-size:1rem}.btn-login,.btn-back{padding:1rem;min-height:52px}}
    ...
    <div class="login-wrap">
        <div class="login-panel">
            ...
            <div>
                ...
                <p class="panel-tagline">Score sheet administration panel.</p>
            </div>
        ...
        <div class="login-form-wrap">
            ...
            <?php endif; ?>
            <form action="<?= site_url('admin/authenticate') ?>" method="post" autocomplete="off">
                ...
                <a href="<?= site_url() ?>" class="btn-back">Back to Site</a>
            </form>
        </div>
    </div>
</body>
